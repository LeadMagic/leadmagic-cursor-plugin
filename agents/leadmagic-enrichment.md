...
1. First run: if the user is new or asks whether they are connected, Cursor should already have prompted **OAuth**. Sign in with the LeadMagic account at [app.leadmagic.io](https://app.leadmagic.io). Then run `check_credit_balance` (and `preview_cost` before paid work). Never ask for an API key.
